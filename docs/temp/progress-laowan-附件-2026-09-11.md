...
> 项目路径: `D:\code\uniapp-unpacked\uniapp`
> 技术栈: UniApp Vue3 + uniCloud-alipay（仅 3 个订单云函数 + order 集合）
> 最后更新: 2026-09-10
> 状态: **既有线上版本运行中；当前手机录入优化已在本地完成，尚未部署**
> 域名: https://env-00jxhanvoaj1-static.normal.cloudstatic.cn/
> 品牌: **老万石雕**

## 待办

- 用 HBuilderX 重新发行当前 H5，并将同一次构建生成的完整 `dist/` 部署到前端网页托管。
- 部署后在真实线上地址冒烟：手机三阶段录入、名单自动规整/撤销、小字悬浮预览、订单口令输错后重输、列表查询与订单保存。

## 2026-09-10 手机录入与名单预览收口

- 独立“老人友好录入”及七步流程已取消，手机新建页恢复为“基本信息 → 名单 → 预览”三阶段；不增加逐步确认，也不增加父母完整日期的额外公历核对。
- 手机名单页使用固定的小字实时预览窗，只展示小字；最终预览页再展示完整碑面。电脑详情页不增加小字浮窗，保留原完整碑面预览。
- 手机和电脑名单区都提供“自动规整 / 撤销整理”。规整按现有辈分、支系和录入顺序匹配夫妻，空白姓名也保留原顺序占位；规整后仍可拖动人工调整。
- 小字生成严格服从当前 `form.names` 的实际分排，不再把相邻夫妻排隐式合并。撤销整理会同步恢复表单、保存载荷和小字预览；继续编辑或拖动后旧撤销快照失效。
- 管理员口令只在接口成功后缓存；返回 401 时清除错误口令并立即重新弹出输入框，用户取消后下次操作仍可再次输入；网络或服务异常不自动重放写请求。
- 最新相关本地提交：`c4069cd`、`488d26d`、`1a18fef`。当前更大范围的老人版定向回退、通用优化和构建产物仍在工作区，尚未整体提交、推送或部署。
- 2026-09-10 重跑现行测试：SFC `5/0`、保存运行时 `7/0`、本地工作流 `11/0`、碑文回归 `16/0`、订单云函数本地模拟 `25/0`，合计 `64 passed, 0 failed`。本轮仅重跑自动化，未重新执行浏览器或真机验收。
