...
function buildRound() {
  const filtered = pinyinData.filter(d => d.unit === store.currentUnit)
  const sampled = sampleWithout(filtered, 10) // 最多 10 题，不足则有多少出多少
  return sampled.map((item, i) => {
    ...
    if (isTypeA) {
      ...
      return { question: item.char, questionType: 'char', options }
    } else {
      const options = shuffle([
        { label: item.char, value: item.char, isCorrect: true },
        ...item.char_distractors.map(d => ({ label: d, value: d, isCorrect: false })),
      ])
      return { question: item.pinyin, questionType: 'pinyin', options }
    }
  })
}

const questions = ref(shuffle(buildRound()))
...
onShow(() => {
  if (roundFinished.value) {
    questions.value = shuffle(buildRound())
